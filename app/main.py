...
app.add_middleware(
    ...
    allow_origins=[
        "*"
    ],  # 开发环境方便，生产环境请指定前端域名，例如: ["http://localhost:8000"]
...
async def transcribe_audio(audio: UploadFile = File(...)):
    ...
    try:
        ...
        print(
            f"STT service responded with error: {e.response.status_code} - {e.response.text}"
        )
        raise HTTPException(
            status_code=e.response.status_code,
            detail=f"STT service error: {e.response.text}",
        )
    except Exception as e:
        print(f"Error forwarding to STT service: {e}")
        raise HTTPException(
            status_code=500, detail=f"Internal server error when calling STT: {e}"
        )
...
async def generate_llm_response(request_body: dict):
    ...
    if not user_text:
        raise HTTPException(
            status_code=400, detail="Missing 'text' in request body for LLM."
        )
    ...
    try:
        ...
        print(
            f"LLM service responded with error: {e.response.status_code} - {e.response.text}"
        )
        raise HTTPException(
            status_code=e.response.status_code,
        ...
        raise HTTPException(
            status_code=500, detail=f"Internal server error when calling LLM: {e}"
        )
